...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        TextView version = label("v0.3.1", 12, BG, true);
    ...
    private void audioLoop() {
        ...
        while (running && recorder != null) {
            ...
            AnalysisResult result = stabilizeResult(analyze(samples));
    ...
    private void renderResult(AnalysisResult result) {
        ...
        spectrumView.setLevels(result.display, result.dominantHz, result.feedback);
        long now = SystemClock.elapsedRealtime();
    ...
    private static class TrendView extends View {
        ...
        void clear() {
            scores.clear(); peaks.clear(); dangers.clear(); invalidate();
        }
        ...
        void addPoint(int score, double peakDb, boolean danger) {
            ...
            peaks.add((float) Math.max(0, Math.min(100, (peakDb + 60.0) * 100.0 / 60.0)));
            dangers.add(danger);
            invalidate();
        }
        ...
        @Override protected void onDraw(Canvas canvas) {
            ...
            for (int i = 1; i < scores.size(); i++) {
                ...
                float sy1 = bottom - scores.get(i - 1) / 100f * (bottom - top);
                float sy2 = bottom - scores.get(i) / 100f * (bottom - top);
                float py1 = bottom - peaks.get(i - 1) / 100f * (bottom - top);
                float py2 = bottom - peaks.get(i) / 100f * (bottom - top);
                canvas.drawLine(x1, sy1, x2, sy2, scorePaint);
                canvas.drawLine(x1, py1, x2, py2, peakPaint);
                if (dangers.get(i)) canvas.drawCircle(x2, sy2, 3.5f * d, dangerPaint);
            }
        }
    }
    ...
    private static class SpectrumView extends View {
        ...
        void setLevels(float[] levels, double peakHz, boolean feedbackPeak) {
            ...
            if (current.length != target.length) current = new float[target.length];
            this.peakHz = peakHz;
        ...
        protected void onDraw(Canvas canvas) {
            super.onDraw(canvas);
            float density = getResources().getDisplayMetrics().density;
            float left = 18 * density;
            float right = getWidth() - 12 * density;
            ...
            for (int i = 0; i < current.length; i++) current[i] += (target[i] - current[i]) * 0.12f;
            if (simpleMode) {
                ...
                for (int i = 0; i < current.length; i++) {
                    ...
                    float y1 = bottom - value * (bottom - top);
                    rect.set(x1, y1, x2, bottom);
                    canvas.drawRoundRect(rect, 3, 3, barPaint);
                }
            }

            if (peakHz >= 35 && peakHz <= 16000) {
                double position = Math.log(peakHz / 35.0) / Math.log(16000.0 / 35.0);
                float x = left + (float) position * (right - left);
                markerPaint.setColor(feedbackPeak ? DANGER : INFO);
                canvas.drawLine(x, top, x, bottom, markerPaint);
            }

            if (!simpleMode) {
                canvas.drawText("40", left, getHeight() - 10, textPaint);
                canvas.drawText("200", left + (right - left) * 0.27f, getHeight() - 10, textPaint);
                canvas.drawText("1k", left + (right - left) * 0.52f, getHeight() - 10, textPaint);
                canvas.drawText("5k", left + (right - left) * 0.76f, getHeight() - 10, textPaint);
                canvas.drawText("16k Hz", right - 38 * density, getHeight() - 10, textPaint);
            }

            if (runningAnimation()) postInvalidateDelayed(70);
        }

        private boolean runningAnimation() {
            for (int i = 0; i < current.length; i++) {
                if (Math.abs(current[i] - target[i]) > 0.01f) return true;
            }
            return false;
        }
    }
